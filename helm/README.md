...
| `db.Type `            | Database type (see below)                          | `mysql`                                         |
| `db.Database`         | Database name                                      | `wiki`                                          |
| `db.User`             | Database username                                  | `wiki`                                          |
| `db.Host`             | External database host                             | `nil`                                           |
| `db.existingSecret`   | Provide an existing `Secret` for database          | `nil`                                           |
| `db.existingSecretKey`| The database password key in the existing `Secret` | `db-password`                                   |
| `db.Port`             | External database port                             | `3306`                                          |
| `db.ssl`              | Enable external database SSL connection            | `false`                                         |
| `db.ca`               | Certificate of Authority content for database      | `nil`                                           |
